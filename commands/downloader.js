...
cmd({
        ...
        async(Void, citel, text) => {
	if (!text) return await citel.send('*Provide Repo Url, Ex:- _.gitclone https://github.com/khadherinc/ELLIE-V2_*') 
...
cmd({
		...
		let buttonMessage = {
                        ...
                        mimetype: 'video/mp4',
                        caption: "  •ELLIEV2• " + Config.caption ,
                    }
                 Void.sendMessage(citel.chat, buttonMessage, { quoted: citel })
            ...
            for (let i of search.all) {
				    ...
				    description: `*•ELLIEV2•* / ${i.timestamp}`
		    		   })
            }
            const sections = [{
...
cmd({
        ...
        async(Void, citel, text) => {
            let yts = require("secktor-pack");
            if (!text) return citel.reply(`Example : ${prefix}yts WhatsApp Bot by •ELLIEV2•`);
            let search = await yts(text);
